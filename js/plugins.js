...
(function ($) {
  ...
  $.supersized = function (options) {
    ...
    base.init = function () {
      ...
      $.supersized.vars.options = $.extend(
        {},
    ...
    base._build = function () {
      ...
      while (thisSlide <= base.options.slides.length - 1) {
        ...
        if (thisSlide == base.options.start_slide - 1) {
          ...
          if (base.options.thumb_links) {
            base.options.slides[thisSlide].thumb
              ? (thumbImage = base.options.slides[thisSlide].thumb)
              : (thumbImage = base.options.slides[thisSlide].image);
            thumbMarkers =
              thumbMarkers +
              '<li class="thumb' +
              thisSlide +
      ...
      if (base.options.thumbnail_navigation) {
        ...
        vars.current_slide - 1 < 0
          ? (prevThumb = base.options.slides.length - 1)
          : (prevThumb = vars.current_slide - 1);
        $(vars.prev_thumb)
          .show()
          .html($("<img/>").attr("src", base.options.slides[prevThumb].image));
        ...
        $(vars.next_thumb)
          .show()
          .html($("<img/>").attr("src", base.options.slides[nextThumb].image));
      }
    ...
    //         }
    //         public async Task LoginAsync(LoginVM data)
    //         {
    ...
    //         public async Task RegisterAsync(RegisterVM data)
    //         {
    //             User user = new()
    //             {
    //                 UserName = data.Email,
    ...
    //             var result = _userManager.CreateAsync(user, data.Password);
    //             if (!result.Result.Succeeded)
    //             {
    //                 throw new Exception(result.Result.Errors.First().Description);
    //             }
    ...
    base._start = function () {
      ...
      if (base.options.start_slide) {
        ...
        vars.current_slide = Math.floor(
          Math.random() * base.options.slides.length,
        ); // Generate random slide number
      }
      ...
      if (base.options.performance == 3) {
        ...
        base.options.performance == 1 ||
        base.options.performance == 2
      ) {
      ...
      if (base.options.slides.length > 1) {
        if (base.options.slides.length > 2) {
          // Set previous image
          vars.current_slide - 1 < 0
            ? (loadPrev = base.options.slides.length - 1)
            : (loadPrev = vars.current_slide - 1); // If slide is 1, load last slide as previous
          var imageLink = base.options.slides[loadPrev].url
            ? "href='" + base.options.slides[loadPrev].url + "'"
            : "";

          var imgPrev = $(
            '<img src="' + base.options.slides[loadPrev].image + '"/>',
          );
          var slidePrev = base.el + " li:eq(" + loadPrev + ")";
          imgPrev
            .appendTo(slidePrev)
            .wrap("<a " + imageLink + linkTarget + "></a>")
            .parent()
            .parent()
      ...
      // Set current image
      imageLink = api.getField("url")
        ? "href='" + api.getField("url") + "'"
        : "";
      var img = $('<img src="' + api.getField("image") + '"/>');

      var slideCurrent = base.el + " li:eq(" + vars.current_slide + ")";
      img
        .appendTo(slideCurrent)
        .wrap("<a " + imageLink + linkTarget + "></a>")
        .parent()
        .parent()
      ...
      if (base.options.slides.length > 1) {
        ...
        vars.current_slide == base.options.slides.length - 1
          ? (loadNext = 0)
          : (loadNext = vars.current_slide + 1); // If slide is last, load first slide as next
        imageLink = base.options.slides[loadNext].url
          ? "href='" + base.options.slides[loadNext].url + "'"
          : "";

        var imgNext = $(
          '<img src="' + base.options.slides[loadNext].image + '"/>',
        );
      ...
      if (base.options.slideshow && base.options.slides.length > 1) {
        ...
        if (base.options.autoplay && base.options.slides.length > 1) {
          vars.slideshow_interval = setInterval(
            base.nextSlide,
            base.options.slide_interval,
          ); // Initiate slide interval
        } else {
      ...
      return base.$el.each(function () {
        ...
        $("img", base.el).each(function () {
          ...
          if (base.options.fit_always) {
            ...
            // Normal Resize
            if (
              browserheight <= base.options.min_height &&
              browserwidth <= base.options.min_width
            ) {
              // If window smaller than minimum width and height

              if (browserheight / browserwidth > ratio) {
                base.options.fit_landscape && ratio < 1
                  ? resizeWidth(true)
                  : resizeHeight(true); // If landscapes are set to fit
              } else {
                base.options.fit_portrait && ratio >= 1
                  ? resizeHeight(true)
                  : resizeWidth(true); // If portraits are set to fit
              }
          ...
          function resizeWidth(minimum) {
            if (minimum) {
              ...
              if (
                base.options.min_height >= browserheight &&
                !base.options.fit_landscape
              ) {
                // If minimum height needs to be considered
                if (
                  browserwidth * ratio >= base.options.min_height ||
                  (browserwidth * ratio >= base.options.min_height &&
                    ratio <= 1)
                ) {
          ...
          function resizeHeight(minimum) {
            if (minimum) {
              ...
              if (base.options.min_width >= browserwidth) {
                ...
                if (
                  browserheight / ratio >= base.options.min_width ||
                  ratio > 1
                ) {
    ...
    base.nextSlide = function () {
      ...
      vars.current_slide + 1 == base.options.slides.length
      ...
      vars.current_slide == base.options.slides.length - 1
        ? (loadSlide = 0)
        : (loadSlide = vars.current_slide + 1); // Determine next slide
      ...
      if (!$(targetList).html()) {
        ...
        imageLink = base.options.slides[loadSlide].url
          ? "href='" + base.options.slides[loadSlide].url + "'"
          : ""; // If link exists, build it
        var img = $(
          '<img src="' + base.options.slides[loadSlide].image + '"/>',
        );
      ...
      if (base.options.thumbnail_navigation == 1) {
        ...
        $(vars.prev_thumb).html(
          $("<img/>").attr("src", base.options.slides[prevThumb].image),
        );

        // Load next thumbnail
        nextThumb = loadSlide;
        $(vars.next_thumb).html(
          $("<img/>").attr("src", base.options.slides[nextThumb].image),
        );
      }
      ...
      // using WebApplication4.Models;
      // using WebApplication4.Services.Interfaces;
      // using WebApplication4.ViewModels.Member;
      ...
      //         public MemberService(AppDbContext context, IFileService fileService)
      //         {
      //             _context = context;
      //             _fileService = fileService;
      //         }

      //         public async Task CreateAsync(MemberCreateVM data)
      //         {
      //             string fileName = await _fileService.UploadAsync(data.Image, "uploads/members");
      //             Member member = new()
      //             {
      //                 Name = data.Name,
      ...
      //             string path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/uploads/members", member.Image);
      //             if (member == null) return;
      //             _context.Members.Remove(member);
      //             _fileService.DeleteAsync(path);
      //             await _context.SaveChangesAsync();
      //         }

      //         public async Task<List<MemberVM>> GetAllAsync()
      //         {
      //             return await _context.Members.Select(m => new MemberVM()
      //             {
      //                 Id = m.Id,
      //                 Name = m.Name,
      //                 Surname = m.Surname,
      ...
      //         public async Task<List<MemberUIVM>> GetAllUIAsync()
      //         {
      //             return await _context.Members.Select(m => new MemberUIVM()
      //             {
      //                 Name = m.Name,
      //                 Surname = m.Surname,
      //                 Image = m.Image
      ...
      //         {
      //             return await _context.Members.Where(m => m.Id == id).Select(m => new MemberVM()
      //             {
      //                 Id = m.Id,
      //                 Name = m.Name,
      //                 Surname = m.Surname,
      ...
      //         public async Task UpdateAsync(MemberUpdateVM data, int id)
      //         {
      //             string fileName = await _fileService.UploadAsync(data.Image, "uploads/members");
      //             Member member = await _context.Members.FirstOrDefaultAsync(m => m.Id == id);
      ...
      switch (base.options.transition) {
        ...
        case "fade": // Fade
          nextslide
            ...
            .animate(
              { opacity: 1, avoidTransforms: false },
        ...
        case "slideTop": // Slide Top
          nextslide
            .css({ top: -base.$el.height(), visibility: "visible" })
            .animate(
              { top: 0, avoidTransforms: false },
              base.options.transition_speed,
              function () {
                base.afterAnimation();
              },
        ...
        case "slideRight": // Slide Right
          nextslide
            .css({ left: base.$el.width(), visibility: "visible" })
            .animate(
              { left: 0, avoidTransforms: false },
              base.options.transition_speed,
              function () {
                base.afterAnimation();
              },
        ...
        case "carouselRight": // Carousel Right
          ...
          liveslide.animate(
            { left: -base.$el.width(), avoidTransforms: false },
            base.options.transition_speed,
          );
          break;
        case 7:
    ...
    //                 return View(data);
    //             }
    //             await _memberService.CreateAsync(data);
    //             return RedirectToAction(nameof(Index));
    //         }

    //         public async Task<IActionResult> Edit(int id)
    //         {
    //             var data =  _memberService.GetAllAsync().Result.FirstOrDefault(m => m.Id == id);
    //             if(data == null)
    //             {
    //                 return NotFound();
    //             }

    //             return View(new MemberUpdateVM
    //             {
    //                 Name = data.Name,
    //                 Surname = data.Surname,
    //                 Email = data.Email,
    ...
    //         {
    //             await _memberService.UpdateAsync(data, id);
    ...
    base._placeSlide = function (place) {
      ...
      if (place == "next") {
        ...
        vars.current_slide - 1 < 0
          ? (loadSlide = base.options.slides.length - 1)
          : (loadSlide = vars.current_slide - 1); // Determine next slide
    ...
    base._origDim = function (targetSlide) {
      targetSlide
        .data("origWidth", targetSlide.width())
        .data("origHeight", targetSlide.height());
    };
    ...
    base.afterAnimation = function () {
      ...
      if (vars.update_images) {
        vars.current_slide - 1 < 0
          ? (setPrev = base.options.slides.length - 1)
    ...
    _init: function () {
      ...
      if (api.options.autoplay) {
        ...
        if (api.options.progress_bar)
          $(vars.progress_bar)
            .stop()
            .css({ left: -$(window).width() }); //  Place progress bar
      }
      ...
      $(vars.tray_button).toggle(
        ...
        function () {
          $(vars.thumb_tray)
            .stop()
            .animate(
              { bottom: -$(vars.thumb_tray).height(), avoidTransforms: true },
              300,
            );
          if ($(vars.tray_arrow).attr("src"))
            $(vars.tray_arrow).attr(
      ...
      if (api.options.thumb_links) {
        ...
        $(vars.thumb_back).click(function () {
          if (vars.thumb_page + vars.thumb_interval > 0) {
            vars.thumb_page =
              Math.floor($(vars.thumb_list).width() / vars.thumb_interval) *
              -vars.thumb_interval;
      ...
      if (jQuery.support.opacity) {
        $(vars.prev_slide + "," + vars.next_slide)
          .mouseover(function () {
        ...
        $(vars.thumb_tray).mousemove(function (e) {
          ...
          if (listWidth > containerWidth) {
            ...
            if (diff > 10 || diff < -10) {
              ...
              diff = parseInt(
                Math.abs(parseInt($(vars.thumb_list).css("left")) - newX),
              ).toFixed(0);
              $(vars.thumb_list)
                .stop()
                .animate(
        ...
        if ($(vars.thumb_list).width() > $(vars.thumb_tray).width()) {
          ...
          if (direction == "next") {
            if (vars.current_slide == 0) {
              ...
              $(".current-thumb").offset().left -
                $(vars.thumb_tray).offset().left >=
              vars.thumb_interval
            ) {
              vars.thumb_page = vars.thumb_page - vars.thumb_interval;
              $(vars.thumb_list)
  ...
  $.fn.lightbox = function (options) {
    ...
    var plugin = {
      ...
      init: function (items) {
        plugin.items = items;
        plugin.selector =
          "lightbox-" + Math.random().toString().replace(".", "");
      ...
      loadImage: function () {
        ...
        plugin.lightbox
          .fadeIn("fast")
          .append('<span class="lightbox-loading"></span>');

        var img = $(
          '<img src="' +
            $(plugin.current).attr("href") +
            '" draggable="false">',
        );
      ...
      bindEvents: function () {
        $(plugin.items).click(function (e) {
          if (
            !$("#lightbox").is(":visible") &&
            ($(window).width() < opts.minSize ||
              $(window).height() < opts.minSize)
          ) {
